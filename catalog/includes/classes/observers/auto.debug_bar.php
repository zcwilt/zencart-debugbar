...
class zcObserverDebugBar extends base
{
    ...
    public function updateNotifyFooterEnd(&$class, $eventID, $paramsArray)
    {
        ...
        if (defined('DEBUG_BAR_SHOW_SESSION') && DEBUG_BAR_SHOW_SESSION === 'true') {
            ...
            if (!empty($_SESSION['customer_id'])) {
                $sessionSummary[] = 'customer_id=' . (int)$_SESSION['customer_id'];
            }
            ...
            if (!empty($_SESSION['currency'])) {
                $sessionSummary[] = 'currency=' . (string)$_SESSION['currency'];
            }
            if (isset($_SESSION['cart']) && is_object($_SESSION['cart']) && method_exists($_SESSION['cart'], 'count_contents')) {
                $sessionSummary[] = 'cart_items=' . (int)$_SESSION['cart']->count_contents();
            }
        ...
        if (defined('DEBUG_BAR_SHOW_REQUEST') && DEBUG_BAR_SHOW_REQUEST === 'true') {
            ...
            if (!empty($_GET['main_page'])) {
                $requestSummary[] = 'main_page=' . (string)$_GET['main_page'];
            }
            ...
            $pageContext = [];
            $pageContext[] = 'page_base=' . ($current_page_base !== null && $current_page_base !== '' ? (string)$current_page_base : 'unknown');
            $pageContext[] = 'main_page=' . (!empty($_GET['main_page']) ? (string)$_GET['main_page'] : 'index');
            $pageContext[] = 'request_type=' . ($request_type !== null && $request_type !== '' ? (string)$request_type : 'unknown');
            $pageContext[] = 'template=' . ($template_dir !== null && $template_dir !== '' ? (string)$template_dir : 'default');
            $pageContext[] = 'language=' . (!empty($_SESSION['language']) ? (string)$_SESSION['language'] : 'unknown');
            $pageContext[] = 'currency=' . (!empty($_SESSION['currency']) ? (string)$_SESSION['currency'] : 'unknown');
            $pageContext[] = 'customer=' . (!empty($_SESSION['customer_id']) ? 'yes' : 'no');
            ...
            $requestHtml = '<details style="margin-top:6px;">'
                ...
                . '<pre style="margin:6px 0 10px;max-height:140px;overflow:auto;white-space:pre-wrap;background:#111827;color:#d1d5db;padding:8px;border:1px solid #374151;">'
        ...
        echo '<div id="zc-debug-bar-toggle" style="display:none;position:fixed;right:12px;bottom:12px;z-index:100000;background:#111827;color:#f9fafb;padding:6px 10px;border:1px solid #374151;border-radius:4px;font:12px/1.4 monospace;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.25);">Show Debug Bar</div>'
            ...
            . '<button type="button" id="zc-debug-bar-expand-all" style="background:#374151;border:1px solid #4b5563;color:#f9fafb;padding:2px 8px;font:12px/1.2 monospace;cursor:pointer;">Expand All</button>'
            . '<button type="button" id="zc-debug-bar-collapse-all" style="background:#374151;border:1px solid #4b5563;color:#f9fafb;padding:2px 8px;font:12px/1.2 monospace;cursor:pointer;">Collapse All</button>'
            . '<button type="button" id="zc-debug-bar-hide" style="background:#374151;border:1px solid #4b5563;color:#f9fafb;padding:2px 8px;font:12px/1.2 monospace;cursor:pointer;">Hide</button>'
            . '</div>'
            . '</div>'
            . $sessionHtml
            . $requestHtml
            ...
            . '<script>'
            . '(function(){'
            . 'var storageKey="zc_debug_bar_hidden";'
            . 'var bar=document.getElementById("zc-debug-bar");'
            . 'var toggle=document.getElementById("zc-debug-bar-toggle");'
            . 'var hideButton=document.getElementById("zc-debug-bar-hide");'
            . 'var collapseAllButton=document.getElementById("zc-debug-bar-collapse-all");'
            . 'var expandAllButton=document.getElementById("zc-debug-bar-expand-all");'
